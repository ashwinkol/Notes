...
**Q.** What is the Diffrence Between No-argument and Default Constructor?
...
Parent parameterized constructor with value 10
Child parameterized constructor with value 10
...
In general terms, a Virtual Machine (VM) is software that creates an environment between the computer platform and the end-user, allowing the user to operate programs. Specifically in its original meaning, a VM creates multiple identical execution environments on a single computer to execute programs.

## Java Virtual Machine (JVM)

Java Virtual Machine (JVM) is a type of VM that runs Java bytecode by creating five identical runtime areas to execute class members. This bytecode is generated by the Java compiler in a format understandable by the JVM.

<div style="text-align:center">

[![Java Data Types](/Java%20Notes/images/JVM.jpg)](https://github.com/ashwinkol/Notes/tree/main/)

</div>

## Types of JVMs

The Java 2SDK, Standard Edition, contains two implementations of the Java Virtual Machine:
...
Runtime areas are where classes are executed by specifying their corresponding names using the command `java <className>`. The Java launcher immediately initiates the Java Runtime Environment for class execution as a layer on top of the operating system. The setup is divided into five Java runtime areas:
...
- All classes' bytecode is loaded and stored in this area.
- Static variables are created here.

### Heap Area

- Main memory of JVM.
- All objects of classes and non-static variables' memory are created here.
- This memory is finite and can be configured during runtime setup.
- It is expandable depending on object creation.
- Method area and heap area are both shareable memory areas.

### Java Stacks Area

- All Java methods are executed here.
- JVM creates two threads by default: Main thread and Garbage Collector Thread.
  - Main thread executes Java methods starting with the main method and creates objects in the heap area.
  - Garbage collector thread destroys unused objects in the heap area.
- For each method execution, a separate block (stack frame) is created in the main thread and destroyed after method execution.

### Program Counter Register Area

- Stores the instruction address for tracking the thread's execution.

### Native Methods Stack Area

- Executes all Java native methods.

### Execution Engine

- Controls all executions happening in the JVM.

### ClassLoader Subsystem

- Responsible for loading classes into the JVM's method area.
...
- ApplicationClassLoader loads classes from the application classpath.
- ExtensionClassLoader loads classes from the extension classpath.
